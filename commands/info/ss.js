...
module.exports = {
  ...
  run: async (bot, message, args) => {
      if(!message.channel.nsfw) return message.channel.send(`This channel must be nsfw for this command`)
      if(message.author.id === '742853857104887820') return message.channel.send(`just stop kanye`)
    message.delete();
    ...
    if (urls.length < 8)
      return message
        .reply(
          ":x: https is too short to reach - 8 limit"
        )
